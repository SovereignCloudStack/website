...
To be on the secure side, you can now run Rookify in preflight mode. In this case you could also execute it directly, i.e. by adding the argument `--migrate` (the `analyze_ceph` module does not break anything):
...
⚠️ **Important:**
It is advised to run any other module in preflight mode only, in order to avoid any real changes.
