...
This blog post shows how the SCS Harbor container registry instance has been migrated 
from one cloud service provider to another using lift and shift cloud migration. Within
the migration work, several improvements have been performed as well, like the Harbor 
...
Swift object storage. We will describe the challenges and issues we faced during this 
process.
...
In addition, Harbor deployment didn't use the ability of Harbor to operate in the high 
...
with version 1.25.6 which has been spawned on SCS-compatible infrastructure, but operated
...
replicas to make sure components are distributed to multiple worker nodes and are HA.
Besides the Harbor database and k-v store, there are two Harbor’s components that are
stateful and their state has to be shared across their replicas: job service and registry.
Harbor offers two ways how to achieve the HA of these components. In the case of job 
...
access mode for PVs is not currently supported the second option has been planned. 
In the case of the registry service, the administrator can again configure shared PVs 
for all pods, or alternatively, external object storage could be used as a registry 
storage backend. Our first choice was to use object store as a backend for image blobs 
...
This SCS Harbor container registry migration scenario uses [Velero](https://velero.io/)
tool which enables moving your Harbor instance as-is from one Kubernetes environment to
another using **backup** and **restore** procedures. Our clusters live in different SCS
...
in Velero. Velero with Restic requires Kubernetes version **1.16** or greater in both
...
Swift object store service does not support application credentials authentication to
...
We stored provided S3 API credentials in the file `\~/.aws/credentials`. This credential
...
contains `endpoint-url` argument that points AWS CLI to the SCS OpenStack Swift object store API.
...
and could be controlled via the handy [Velero CLI tool](https://velero.io/docs/main/basic-install/).
If your environment is Linux distribution you can use the following steps and install
...
clusters (Cluster_A and Cluster_B). The following command created a namespace called
`velero`, and placed a deployment named `velero` in it.
...
`EnableAPIGroupVersions` feature has been enabled as well. It is not unusual to see the
...
section to be aware of a potential impact on the Harbor instance. 
...
- The key-value database (Redis) storage should not be migrated (backed up and then restored). 
  As a result, the user sessions of logged users that are stored in Redis will be lost.
...
- The upload purging process may cause backup failure. It is a background process of the
...
After the preparation and installing all prerequisites, we were ready to migrate SCS Harbor
...
It protected our Harbor instance from deleting repository, artifact, tag, and pushing images.
This ensured that the Harbor instance in Cluster_A will be in sync with the Harbor instance
in Cluster_B after we will restore the Harbor instance from the backup in Cluster_B.
...
Velero backup command below successfully created a full backup of all resources in the
...
Besides this issue, we found all Harbor services up and running and after brief control
of services logs we declared migration work as done. 
This new instance of SCS container registry running in Cluster_B was not publicly 
accessible yet as the DNS record still pointed users to the Cluster_A IP address and 
more importantly, this new instance had still outdated Harbor version and was still 
affected by the security issue mentioned above. 
The upgrade to the latest stable Harbor version should have resolved all mentioned issues.
Let's proceed.
...
also enables installation of dependent pieces of software e.g. ingress-controller, 
...
up Swift object store for registry service yet. It just upgraded the Harbor version and 
as a side effect, the upgrade process also deployed missing ingress resource to the 
Cluster_B. So after the successful upgrade to harbor-helm v1.11.1 (Harbor v2.7.1) we 
checked that everything was up and running, and then we adjusted the ingress resource
...
As we set Harbor in Cluster_A to ReadOnly when doing the backup, the restored instance
in Cluster_B was in ReadOnly too. In order to test the whole functionality the new 
instance has been set back from ReadOnly mode. We tested things like creating a project,
creating a user, access to the created project, push/pull images using created user 
to/from the created project, etc.

## Final tuning

After that, just two things were missed. Upgrade Harbor services to HA mode and as a 
prerequisite to this redirect job service persistent storage from PV to Harbor database
and also used Swift object store as persistent storage for registry service instead of PV.

To achieve the above we did several actions. The first thing was to create a new object
...
adjusted storage settings. Visit the [harbor-config](https://github.com/SovereignCloudStack/k8s-harbor/blob/main/envs/public/harbor-config.yaml)
file for the public environment in the k8s-harbor repository to review the config. 
After the upgrade, Harbor services have been deployed with 2 replicas, the job service
used the Harbor database as storage, and the registry service has been pointed to the 
object store bucket. The PV of the registry service that contained all image blobs was 
...
Using this pod we copied all image blobs from PV to the object store.
...
Once the sync has been completed we again did a test round of the whole Harbor instance, 
now deployed (partially) in HA mode (Harbor database and k-v store were out of scope this time).

## Hello new instance

Finally, the new instance was in the desired state. The last steps were to reconfigure 
DNS A record for DNS name `registry.scs.community` from Cluster_A IP address to 
Cluster_B IP address. Right after that we also reconfigured the ingress resource in 
Cluster_B to serve the host `registry.scs.community`. 
This final reconfiguration caused a short outage of the SCS container registry service. 
