...
Das Argument `-s` oder `--show-states` wurde hinzugefügt, um den Fortschritt zu verfolgen. Wenn Sie eine Pickle-Datei zur Konfiguration hinzugefügt haben, wird auch diese Datei berücksichtigt: Jedes einzelne Modul prüft den aktuellen Migrationsstatus und berichtet darüber, indem es ein Log an stdout sendet.
...
📝 **Hinweis:**
Stellen Sie sicher, dass Rookify eine Verbindung zum Testbett herstellen kann. Weitere Informationen zum Einrichten einer VPN-Verbindung finden Sie in der [OSISM-Dokumentation](https://osism.tech/docs/guides/other-guides/testbed/#vpn-access).
...
3. **Rookify ausführen**: Jetzt können Sie endlich Rookify ausführen, um es zu testen. Rookify erlaubt die Verwendung von `--dry-run`, um Module im Preflight-Modus auszuführen. Beachten Sie, dass Rookify die verschiedenen Module immer zuerst im Preflight-Modus ausführt, bevor sie tatsächlich ausgeführt werden. 

Wenn alles korrekt eingerichtet ist, können Sie Rookify ohne Argumente ausführen oder sie wagen es explizit `--migrate` als Argument zu geben. In diesem Fall machen Sie nichts kaput, wenn Sie das `analyze_ceph` Module auszuführen, da dieses keine kritischen Änderungen vornimmt:

📝 **Hinweis:**
  Ohne Argumente wird Rookify per default im Preflight-Moduees laufen (es wird dann `--dry-run` hinzugefügt).
...
Wir können rookify zunächst mit `pre-flight` starten, um zu prüfen, ob alles in Ordnung ist, und es dann explizit mit dem Argumente `--migrate` versehen, um die wirkliche Migration zu starten. Als Ergebnis sollten wir folgenden Output sehen:
